 ...
 *
 */
public abstract class AdapterRuleChoicePage extends SecurityPage implements
	...
	public abstract AdapterRule getAdapterRule();

	public abstract void setAdapterRule(AdapterRule adapterRule);
	...
	public IBasicTableModel getChooseMsgBoard() {
		return new IBasicTableModel() {
			public int getRowCount() {
				return new Long(getAdapterService().findAdapterRuleCount(
	...
	public void chooseSubmit(IRequestCycle cycle) {
		...
		if (getAdapterRule() == null) {
			delegate.setFormComponent(null);
			delegate.record("必须选择一个资源(或资源已被删除)", null);

		} else {
			setRadioValue(String.valueOf((getAdapterRule().getId())));
		}
	}
		...
		return new SimpleTableColumn("adapterType", "适配类型",
				new ITableColumnEvaluator() {
					...
					public Object getColumnValue(ITableColumn objColumn,
							Object objRow) {
						AdapterRule adapterRule = (AdapterRule) objRow;
						if (adapterRule.getAdapterTypeId() != null) {

							AdapterType adapterType = getAdapterService()
									.getAdapterTypeById(
											adapterRule.getAdapterTypeId());
							if (adapterType == null)
								return "";
	...
	public ITableColumn getAdapter() {
		return new SimpleTableColumn("adapter", "适配器",
				new ITableColumnEvaluator() {
					...
					public Object getColumnValue(ITableColumn objColumn,
							Object objRow) {
						AdapterRule adapterRule = (AdapterRule) objRow;
						if (adapterRule.getAdapterId() != null) {

							Adapter adapter = getAdapterService()
									.getAdapterById(adapterRule.getAdapterId());
							if (adapter == null)
								return "";
