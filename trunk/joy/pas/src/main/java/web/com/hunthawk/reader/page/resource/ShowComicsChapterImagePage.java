...
public abstract class ShowComicsChapterImagePage extends EditPage implements
		PageBeginRenderListener {
	@InjectObject("spring:resourceService")
	public abstract ResourceService getResourceService();

	public abstract ComicsChapter getComicsChapter();

	public abstract void setComicsChapter(ComicsChapter comicsChapter);
	...
	public boolean isCheckImage() {
		ComicsChapter chapter = getComicsChapter();
		if(chapter!=null){
	...
	// 上一张
	public IPage previousImage(ComicsChapter comicsChapter, String imageIndex) {
		Integer index = 0;
		Integer images = comicsChapter.getImages().length; // 得到所有的图片数量
		if (imageIndex != null) {
		...
		this.setComicsChapter(comicsChapter);
		this.setModel(comicsChapter);
		return this;
	}

	// 下一张
	public IPage nextImage(ComicsChapter comicsChapter, String imageIndex) {
		Integer index = 0;
		Integer images = comicsChapter.getImages().length - 1; // 得到所有的图片数量
		if (imageIndex != null) {
	 ...
	 * 图片显示序号
	 */
	public IPage deleteImage(ComicsChapter comicsChapter, String imageIndex) {
		Integer index = 0;
		Integer images = comicsChapter.getImages().length - 1; // 得到所有的图片数量
		String[] imageList =  comicsChapter.getImages();//存放图片的数组
		String inmageName = comicsChapter.getImageName();//得到图片的存放的字符串
		...
		this.setImageIndex(index.toString());
		comicsChapter.setImageName(inmageName);
		try {
