...
public abstract class ShowNewsPapersChapterImagePage extends EditPage implements
		PageBeginRenderListener {
	@InjectObject("spring:resourceService")
	public abstract ResourceService getResourceService();

	public abstract NewsPapersChapter getNewsPapersChapter();

	public abstract void setNewsPapersChapter(
			NewsPapersChapter newsPapersChapter);
	...
	public boolean isCheckImage() {
		NewsPapersChapter chapter = getNewsPapersChapter();
		if(chapter!=null){
	...
	public void pageBeginRender(PageEvent arg0) {
		if (getModel() == null) {
			setModel(new NewsPapersChapter());
		}
	...
	// 上一张
	public IPage previousImage(NewsPapersChapter newsPapersChapter,
			String imageIndex) {
		Integer index = 0;
		Integer images = newsPapersChapter.getImages().length; // 得到所有的图片数量
		if (imageIndex != null) {
		...
		this.setNewsPapersChapter(newsPapersChapter);
		this.setModel(newsPapersChapter);
		return this;
	}

	// 下一张
	public IPage nextImage(NewsPapersChapter newsPapersChapter, String imageIndex) {
		Integer index = 0;
		Integer images = newsPapersChapter.getImages().length - 1; // 得到所有的图片数量
		if (imageIndex != null) {
	 ...
	 * 图片显示序号
	 */
	public IPage deleteImage(NewsPapersChapter newsPapersChapter, String imageIndex) {
		Integer index = 0;
		Integer images = newsPapersChapter.getImages().length - 1; // 得到所有的图片数量
		String[] imageList =  newsPapersChapter.getImages();//存放图片的数组
		String inmageName = newsPapersChapter.getImageName();//得到图片的存放的字符串
		...
		this.setImageIndex(index.toString());
		newsPapersChapter.setImageName(inmageName);
		try {
