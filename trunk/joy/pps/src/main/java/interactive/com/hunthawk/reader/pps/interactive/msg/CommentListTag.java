...
/**
 * 留言列表标签 标签名称：comment_list 参数说明： pageSize: 每页显示的条数 (3、10、20)
 * noPageLink:不显示翻页相关的链接 boardId： 板块ID msgType(*)：留言类型 参考MsgRecord定义的 1.内容 2.栏目
 * 3.产品 4.用户定制 id:用户订制ID号 title:返回链接文字 showBack:是否显示返回链接
 * 
 * @author liuxh
 * 
 */
public class CommentListTag extends BaseTag {
	...
	public Map parseTag(HttpServletRequest request, String tagName) {
		...
		int msgType = request.getParameter(ParameterConstants.COMMENT_TARGET) == null ? getIntParameter(
				"msgType", -1)
				: Integer.parseInt(request
						.getParameter(ParameterConstants.COMMENT_TARGET));
		int boardId = request.getParameter(ParameterConstants.COMMENT_PLATE) == null ? getIntParameter(
				"boardId", -1)
				: Integer.parseInt(request
						.getParameter(ParameterConstants.COMMENT_PLATE));
		...
		try {
			...
			TagLogger.debug(tagName, "[标签名=" + tagName
					+ ",出错原因=currentPage转整型时异常]", request.getQueryString(), e);
		}
		if (msgType == MsgRecord.TYPE_CUSTOM) {// 用户定制
			id = id == null ? getParameter("id", "") : id;
			if (StringUtils.isEmpty(id)) {
				TagLogger.debug(tagName, "留言类型为用户定制时，ID属性值为空", request
						.getQueryString(), null);
				return new HashMap();
			}
			msgRecords = getInteractiveService(request).getMsgRecordList(
					msgType, boardId, null, null, null, id,
					isNoPageLink() ? 1 : currentPage, pageSize);
			msgCount = getInteractiveService(request).getMsgRecordCount(
					msgType, boardId, null, null, null, id);
		} else if (msgType == MsgRecord.TYPE_CONTENT) {// 内容
			id = id == null ? URLUtil.getResourceId(request) : id;
			msgRecords = getInteractiveService(request).getMsgRecordList(
					msgType, boardId, id, null, null, null,
					isNoPageLink() ? 1 : currentPage, pageSize);
			msgCount = getInteractiveService(request).getMsgRecordCount(
					msgType, boardId, id, null, null, null);
		} else if (msgType == MsgRecord.TYPE_COLUMN) {// 栏目
			id = id == null ? request
					.getParameter(ParameterConstants.COLUMN_ID) : id;
			msgRecords = getInteractiveService(request).getMsgRecordList(
					msgType, boardId, null, Integer.parseInt(id), null, null,
					isNoPageLink() ? 1 : currentPage, pageSize);
			msgCount = getInteractiveService(request).getMsgRecordCount(
					msgType, boardId, null, Integer.parseInt(id), null, null);
		} else if (msgType == MsgRecord.TYPE_PRODUCT) {// 产品
			id = id == null ? request
					.getParameter(ParameterConstants.PRODUCT_ID) : id;
			msgRecords = getInteractiveService(request).getMsgRecordList(
...
//		}
		...
		back.append(URLUtil.removeParameter(request.getQueryString(),
				ParameterConstants.PAGE_NUMBER, ParameterConstants.TEMPLATE_ID,
