...
public class LookbackEverTag extends BaseTag {
	...
	public Map parseTag(HttpServletRequest request, String tagName) {
		...
		if(StringUtils.isEmpty(resourceId)){
			TagLogger.debug(tagName, "往期回顾标签获取不到资源ID", request.getQueryString(), null);
			return new HashMap();
		}
		...
		try{
			...
			ex.printStackTrace();
			TagLogger.debug(tagName, "获取往期资源列表失败", request.getQueryString(), ex);
			return new HashMap();
		}
		...
		try {
			...
			e.printStackTrace();
			TagLogger.debug(tagName, "获取往期资源列表失败", request.getQueryString(), e);
			return new HashMap();
		}
