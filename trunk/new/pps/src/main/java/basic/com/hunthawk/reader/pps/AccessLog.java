...
public class AccessLog {
	...
	public static void log(HttpServletRequest request, Integer bytes,
		...
		if(url.indexOf(ParameterConstants.UNICOM_PT)<=0 && url.indexOf("s.py")>0){
			String PT = (String)request.getSession().getAttribute("UNICOM_PT");
			if(StringUtils.isNotEmpty(PT)){
				url += "&"+ParameterConstants.UNICOM_PT+"="+PT;
			}
		}
