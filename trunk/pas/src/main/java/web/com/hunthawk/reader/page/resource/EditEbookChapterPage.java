...
public abstract class EditEbookChapterPage extends EditPage implements
	...
	protected boolean persist(Object object) {
		try {
			EbookChapter chapter = (EbookChapter) object;
			String errorMessage = "";

			if (getResourceAll() != null)
				chapter.setResourceId(getResourceAll().getId());
			if (chapter.getBContent() != null
					&& !"".equals(chapter.getBContent()))
				chapter.setChapterSize(chapter.getBContent().length());
			...
			{	
				...
				List<EbookChapter> chapterList = getResourceService().getResourceChapter(EbookChapter.class, getResourceAll().getId());//得到按照序号排序的List集合
				int i=0;
				int totalsize=0;
			...
			chapter.setImageNum(chapter.getImages().length);
			getResourceService().updateResourceChapter(chapter);
	...
	public String getPreAddress() {
		// 得到资源中的图片
		EbookChapter chapter = (EbookChapter) getModel();
