...
public abstract class EditMagazinePage extends EditPage implements
		...
		try {
			...
			if (!isModelNew()) {
				...
				if (copyrighId != null && !"".equals(copyrighId)) {
					if (!copyrighId.equals(magazine.getCopyrightId())) { // 表明修改了
						magazine.setLastCopyrightId(magazine.getCopyrightId());
						magazine.setCopyrightId(copyrighId);
					}
				}
				if(selectedResourceType.size()==0)
					throw new Exception("请选择资源分类！");
				UserImpl user = (UserImpl) getUser();
				magazine.setModifierId(user.getId());
				getResourceService().updateResource(magazine,
						ResourceAll.RESOURCE_TYPE_MAGAZINE);
				// 根据资源id将引用关系先删除
				getResourceService().deleteResourceResType(magazine.getId());
				// 添加资源与资源列表关系表
	...
	public Collection<HibernateExpression> getSearchExpressions() {
		...
		HibernateExpression nameE = new CompareExpression("showType", 3,
				CompareType.Equal);
	...
	public List getSelectedResourceTypes() {
		if (isModelNew()) {// 新增加
			...
			Magazine magazine = (Magazine) getModel();
			// ebook.getId();
			Collection<HibernateExpression> expressions = new ArrayList<HibernateExpression>();
			HibernateExpression ex = new CompareExpression("rid", magazine
	...
	public IPage showChapter(ResourceAll resourceAll) {
		getShowMagazineChapterPage().setResourceAll(resourceAll);
		return getShowMagazineChapterPage();
