...
public abstract class CommentLinkGuide extends SecurityPage implements
		IExternalPage {
		...
		if (getParameterValue() != null) {
			...
			for (int i = 0; i < strs.length; i++) {
				...
				if (kv.length == 2) {
					...
					} else if ("customName".equals(kv[0])) {
						
							setCustomName(kv[1]);
						
					} else if ("isShowCount".equals(kv[0])) {
						Integer iv = getInteger(kv[1]);
						if (iv != -999) {
							setIsShowCount(iv);
						}
					} else if ("templateIdComment".equals(kv[0])) {
						Integer iv = getInteger(kv[1]);
						if (iv != -999) {
							setTemplateIdComment(iv);
						}
