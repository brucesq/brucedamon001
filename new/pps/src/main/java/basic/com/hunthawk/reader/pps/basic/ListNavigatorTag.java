...
public class ListNavigatorTag extends BaseTag {
				...
				if (style.equals("3")) {// 跳转
					...
					for (int i = 0; i < pams.length; i++) {
						...
						if (str.length > 1 && str[1] != null
							...
							// obj.put("value", str[1]);
							try {
								obj.put("value", java.net.URLDecoder.decode(
										str[1], "utf-8"));
					...
					String result = DBVmInstance.getInstance().parseVM(
							velocityMap, this, tagTem);
					...
					res.put(ParameterConstants.PRE_TAG_SUFFIX + tagName
							+ ParameterConstants.END_TAG_SUFFIX, makeNaviPage(
							request, black));
					return res;
				} else if (style.equals("4")) {

					Map velocityMap = new HashMap();
					velocityMap.put("objs", makeNaviPageObjs(request, black));
					int num = getIntParameter("gonum", 1);
					Map map = makeUpDownPage(request, black,num);
					velocityMap.put("flag", flag);
					velocityMap.put("prelink", (map.get("prelink") == null ? ""
							: map.get("prelink")));
					velocityMap.put("nextlink",
	...
	private List makeNaviPageObjs(HttpServletRequest request, List<String> black) {
		Navigator navi = (Navigator) request
				.getAttribute(ParameterConstants.PAMS_NAVIGATOR);

		Map<String, String> para = new HashMap<String, String>();
		para.put(ParameterConstants.PAGE_NUMBER, String.valueOf(curPage));
//		int minIdx = curPage+1;//navi.getMinindex() + 1;
//		int maxIdx = curPage+5;//navi.getMaxindex() ;
		
		int minIdx = (curPage/5)  ;
		minIdx = minIdx*5+1;
		int maxIdx = minIdx+4;
		
		if(minIdx >  navi.getPagecount() -4 ){
			minIdx = navi.getPagecount() -4;
		}
		...
		if(maxIdx > navi.getPagecount()){
			maxIdx = navi.getPagecount();
		}
		...
		for (int i = minIdx; i <= maxIdx; ++i) {
			...
			String url = URLUtil.urlChange(request, para, black);
			String title = "" + i;
			Map<String, String> obj = new HashMap<String, String>();
			obj.put("title", title);
			obj.put("url", url);
	...
	private Map makeUpDownPage(HttpServletRequest request, List<String> black,
		...
		int minIdx = (curPage/5)  ;
		minIdx = minIdx*5+1-5;
		int maxIdx = minIdx+10;
		
		if (maxIdx < pageCount+1) {
			para.clear();
			para.put(ParameterConstants.PAGE_NUMBER, "" + maxIdx);
		...
		if (minIdx > 1) {
			para.clear();
			para.put(ParameterConstants.PAGE_NUMBER, "" + minIdx);
