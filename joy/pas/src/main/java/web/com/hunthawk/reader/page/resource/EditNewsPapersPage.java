...
public abstract class EditNewsPapersPage extends EditPage implements
			...
			if (!isModelNew()) {
				...
				if (copyrighId != null && !"".equals(copyrighId)) {
					if (!copyrighId.equals(newsPapers.getCopyrightId())) { // 表明修改了
						newsPapers.setLastCopyrightId(newsPapers
								.getCopyrightId());
				...
				getResourceService().updateResource(newsPapers,
	...
	public Collection<HibernateExpression> getSearchExpressions() {
		...
		HibernateExpression nameE = new CompareExpression("showType", 2,
				CompareType.Equal);
	...
	public List getSelectedResourceTypes() {
		if (isModelNew()) {// 新增加
			...
			HibernateExpression ex = new CompareExpression("rid", newsPapers
	...
	@InjectPage("resource/ShowNewsPapersChapterPage")
	public abstract ShowNewsPapersChapterPage getShowNewsPapersChapterPage();

	public IPage showChapter(ResourceAll resourceAll) {
		getShowNewsPapersChapterPage().setResourceAll(resourceAll);
		return getShowNewsPapersChapterPage();
