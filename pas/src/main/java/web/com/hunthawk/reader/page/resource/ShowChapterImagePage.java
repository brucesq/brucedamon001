...
public abstract class ShowChapterImagePage extends EditPage implements
	...
	public String  reImageName(String imageName){
		...
		Integer number = Integer.parseInt(str2[0]);//得到了当前的图片的排序序号
	...
	public IPage previousImage(EbookChapter ebookChapter, String imageIndex) {
		Integer index = 0;
		Integer images = ebookChapter.getImages().length; // 得到所有的图片数量
		if (imageIndex != null) {
	...
	public IPage nextImage(EbookChapter ebookChapter, String imageIndex) {
		Integer index = 0;
		Integer images = ebookChapter.getImages().length - 1; // 得到所有的图片数量
		if (imageIndex != null) {
	...
	public IPage deleteImage(EbookChapter ebookChapter, String imageIndex) {
		Integer index = 0;
		Integer images = ebookChapter.getImages().length - 1; // 得到所有的图片数量
		String[] imageList =  ebookChapter.getImages();//存放图片的数组
		String inmageName = ebookChapter.getImageName();//得到图片的存放的字符串
