...
/**
 * 收藏标签(链接) 标签名称：favorite_link 参数说明： property:1.添加 -1.删除 title:链接名称
 * templateId:模板ID isDel:是否做删除操作的开关钮 等待确认删除 number:在第几页之前显示 add by liuxh
 * 2009-9-2 split:分割符号
 * 
 * @author liuxh
 * 
 */
public class FavoriteLinkTag extends BaseTag {
	...
	public Map parseTag(HttpServletRequest request, String tagName) {
		...
		String title = getParameter("title", "加入收藏");// 链接文字
		...
		if (property > 0) {// 添加链接
			...
			sb.append("=");
			sb.append(ParameterConstants.COMMON_PAGE_FAVORITE_ADD);
			sb.append("&");
		} else {// 删除链接
			title = "我要删除";
			if (isDel > 0) {
