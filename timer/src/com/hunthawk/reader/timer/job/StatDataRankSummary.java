...
public class StatDataRankSummary {
	...
	protected static Logger logger = Logger.getLogger(StatDataRankSummary.class);
...
//		}
	...
	private void universalMethod(String hql, String type, Object obj) {
		...
		while (hasResult) {
			...
			logger.info("summart:"+type+":"+clickResult.size() +":"+new Date());
			if (clickResult.size() < pageSize) {
				hasResult = false;
			} else {
				pageNo++;
...
//			Transaction tx = session.beginTransaction();
			...
			for (Object[] objs : clickResult) {
				...
				ResourceAll resource = controller.get(ResourceAll.class, objs[0].toString());
//				ResourceAll resource = (ResourceAll) session.get(
//						ResourceAll.class, objs[0].toString());
				if (resource != null) {
...
//						resource.setVoteNum(((Long) objs[1]).intValue()); // 设置投票汇总
						...
						logger.info("DT"+resource.getId()+":"+resource.getRankingNum());
